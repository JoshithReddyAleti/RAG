...
def process_file(file_path):
    ...
    try:
        ...
        print(f"Error in process_file for {file_path}: {e}")
        return [], []
...
def store_chunks(chunks, metadata, json_path):
    ...
    try:
        ...
        print(f"Stored {len(chunks)} chunks to {json_path} and ChromaDB")
    except Exception as e:
        print(f"Error storing chunks for {json_path}: {e}")
    ...
    try:
        ...
        while True:
            ...
            is_file_specific = any(
                fname.lower() in query.lower() for fname in os.listdir("./upload")
            )
